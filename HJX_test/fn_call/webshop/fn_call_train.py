...
def main():
    ...
    def react_traj_to_fn_call(example):
        ...
        for i in range(2, len(conversations)):
            if conversations[i]["from"] == "gpt":
                conversations[i]["value"] = WebshopAdapter.to_function_calling(
    ...
    trainer = Trainer(
        ...
        args=TrainingArguments(
            output_dir="/root/AgentGym/HJX_test/fn_call/outputs/model",
            do_train=True,
            ...
            warmup_ratio=0.1,
            logging_dir="/root/AgentGym/HJX_test/fn_call/outputs/logs",
