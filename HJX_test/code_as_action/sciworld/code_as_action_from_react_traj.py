...
def main():
    ...
    def react_traj_to_fn_call(example):
        ...
        for i in range(2, len(conversations)):
            if conversations[i]["from"] == "gpt":
                try:
                    conversations[i]["value"] = SciWorldAdapter.to_code_as_action(
    ...
    dataset = datasets.load_dataset(
        "json",
        data_files="/mnt/data/user/huang_jixuan/agentgym/AgentTraj-L/sciworld_train.json",
    ).map(
    ...
    trainer = Trainer(
        ...
        args=TrainingArguments(
            output_dir="/root/AgentGym/HJX_test/code_as_action/outputs/models/sciworld_qwen2_0.5b_instruct",
            do_train=True,
            ...
            warmup_ratio=0.1,
            logging_dir="/root/AgentGym/HJX_test/code_as_action/outputs/logs/sciworld_qwen2_0.5b_instruct",
